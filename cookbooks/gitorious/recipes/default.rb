%w( apg libyaml-devel GeoIP GeoIP-devel libssh2 libssh2-devel openssh openssh-server memcached libyaml libyaml-devel ImageMagick ImageMagick-devel apr-devel uuid java-1.6.0-openjdk readline-devel glibc-devel openssl-devel gcc-c++ gcc-c++ make zlib-devel openssl-devel readline-devel sphinx ).each { |p| package p }

include_recipe "passenger_apache2::mod_rails"
apache_module "rewrite"
include_recipe "apache2::mod_ssl"
include_recipe "mysql::server"
#%w( apg libyaml-devel GeoIP GeoIP-devel libssh2 libssh2-devel openssh openssh-server memcached libyaml libyaml-devel 
#ImageMagick ImageMagick-devel apr-devel uuid java-1.6.0-openjdk readline-devel glibc-devel openssl-devel gcc-c++ gcc-c++ make zlib-devel openssl-devel readline-devel sphinx mod_ssl ).each { |p| package p }

# comment the above line if you want to keep portmap and rpcbind
#%w( portmap rpcbind ).each {|p| package(p) { action :purge } }

gem_package 'raspell' do
   version("1.0")
end
...
web_app "gitorious" do
  ...
  server_aliases node[:gitorious][:host]
  cookbook "passenger_apache2"
  rails_env "production"
end

template "/etc/httpd/sites-available/gitorious-ssl" do
...
['ultrasphinx', 'daemon', 'poller', 'stomp'].each do |daemon|
